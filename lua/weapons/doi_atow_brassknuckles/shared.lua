...
if CLIENT then
	...
	SWEP.WM = "models/khrcw2/doipack/w_brassknuckles.mdl"
	SWEP.WMPos = Vector(-2.25, 3, -.3)
	SWEP.WMAng = Vector(-5, 25, 180)
	
	SWEP.SwimPos = Vector(0, 0, 0)
	SWEP.SwimAng = Vector(-40.8947, 0, 0)
	
	SWEP.SprintPos = Vector(0, 0, 0)
	SWEP.SprintAng = Vector(-40.8947, 0, 0)
	...
	killicon.Add( "doi_atow_brassknuckles", "vgui/inventory/killicons/weapon_brassknuckles", Color(255, 120, 40, 0))
end


SWEP.Animations = {
	slash_primary = {"swing3"},
	slash_secondary = {"swing1","swing2"},
	draw = "draw_1"
}

SWEP.Sounds = {
    swing1 = {{time = 0.02, sound = "DOIBRASS_SWING"}},
	swing2 = {{time = 0.02, sound = "DOIBRASS_SWING"}},
	swing3 = {{time = 0.02, sound = "DOIBRASS_SWING"}},
	draw_1 = {{time = 0, sound = "DOIBRASS_DRAW1"},
	{time = .2, sound = "DOIBRASS_DRAW2"},
	},
}
...
SWEP.ViewModel = "models/khrcw2/doipack/brassknuckles.mdl"
SWEP.WorldModel = "models/khrcw2/doipack/w_brassknuckles.mdl"
...
SWEP.SecondaryAttackDelay = 0.65

SWEP.PrimaryAttackDamage = {50, 50}
SWEP.SecondaryAttackDamage = {60, 60}

SWEP.PrimaryAttackRange = 50
SWEP.SecondaryAttackRange = 50

SWEP.HolsterTime = 0.15
SWEP.DeployTime = 0.2

SWEP.PrimaryAttackImpactTime = 0.08
SWEP.PrimaryAttackDamageWindow = 0.1

SWEP.SecondaryAttackImpactTime = 0.11
...
SWEP.PushVelocity = 25

SWEP.PlayerHitSounds = {"DOIBRASS_HITFLESH"}
SWEP.NPCHitSounds = {"DOIBRASS_HITFLESH"}
SWEP.MiscHitSounds = {"DOIBRASS_HITWORLD"}
...
function SWEP:IndividualThink()
	if (SP and SERVER) or IsFirstTimePredicted() then
		...
		if self.attackDamageTime and ct > self.attackDamageTime and ct < self.attackDamageTime + self.attackDamageTimeWindow then
			...
			self.Owner:ViewPunch(Angle(0, 0, -.05))
			...
			if trace.Hit then
				...
				if IsValid(ent) then
					...
					self.Owner:ViewPunch(Angle(-1, 0, -.25))
...
function SWEP:SecondaryAttack()
	...
	if IsFirstTimePredicted() then
	self.Owner:ViewPunch(Angle(-1, 1, -2))
