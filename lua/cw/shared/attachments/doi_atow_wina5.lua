local att = {}
att.name = "doi_atow_wina5"
att.displayNameShort = "W.A5 7x"
att.displayName = "Winchester A5"
att.aimPos = {"WinA5Pos", "WinA5Ang"}
att.FOVModifier = 20
...
if CLIENT then
	...
	att.description = {
		[1] = {t = "American 7x magnification scope.", c = CustomizableWeaponry.textColors.POSITIVE},
		[2] = {t = "For long range.", c = CustomizableWeaponry.textColors.POSITIVE}
	}

	local old, x, y, ang
	local reticle = surface.GetTextureID("models/khrcw2/doipack/enfield/enfield_crosshair")
...
function att:drawRenderTarget()
		...
		if self.NO32AxisAlign then
			local right, up, forward = ang:Right(), ang:Up(), ang:Forward()
			
			ang:RotateAroundAxis(right, self.NO32AxisAlign.right)
			ang:RotateAroundAxis(up, self.NO32AxisAlign.up)
			ang:RotateAroundAxis(forward, self.NO32AxisAlign.forward)
		end
